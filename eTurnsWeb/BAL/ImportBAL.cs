...
{
    public class ImportBAL : IDisposable
    {
        #region Properties
        ...
        public ImportBAL(Action clearCurrentResourceList)
        {
        ...
        public void SaveItemSupplier(List<UDFOptionsCheckDTO> lst, List<UDFOptionsMain> CurrentOptionList,
            ...
            bool IsFirstCall, ref string message, ref string status, ref bool allSuccesfulRecords)
        {
...

            {
                ...
                if (LstItemSuppliermain != null && LstItemSuppliermain.Length > 0)
                {
                    ...
                    {
                        lstreturn = objImportDAL.BulkInsert(ImportMastersDTO.TableName.ItemSupplierDetails.ToString(), CurrentItemSupplierList, SessionHelper.RoomID, SessionHelper.CompanyID, HttpContext.Current.Session["ColuumnList"].ToString(), SessionHelper.UserID, SessionUserId, SessionHelper.EnterPriceID, CurrentOptionList);
                    }
        ...
        public void SaveLocationMaster(List<UDFOptionsCheckDTO> lst, List<UDFOptionsMain> CurrentOptionList,
           ...
           ImportMastersDTO.TableName TableName, string para, bool HasMoreRecords,
           bool IsFirstCall, ref string message, ref string status, ref bool allSuccesfulRecords, string currentCulture)
        {
            long SessionUserId = SessionHelper.UserID;
            JavaScriptSerializer serializer = new JavaScriptSerializer();
            serializer.MaxJsonLength = int.MaxValue;
...

            {
                ...
                if (LstLocationMaster != null && LstLocationMaster.Length > 0)
                {
                    ...
                    lst = objImportDAL.GetAllUDFList(SessionHelper.RoomID, ImportMastersDTO.TableName.LocationMaster.ToString());

                    eTurns.DAL.UDFDAL objUDFDAL = new eTurns.DAL.UDFDAL(SessionHelper.EnterPriseDBName);
                    IEnumerable<UDFDTO> UDFDataFromDB = objUDFDAL.GetRequiredUDFsByUDFTableNamePlain("LocationMaster", SessionHelper.RoomID, SessionHelper.CompanyID);
                    ...
                    {
                        lstreturn = objImportDAL.BulkInsert(ImportMastersDTO.TableName.LocationMaster.ToString(), CurrentLocationList, SessionHelper.RoomID, SessionHelper.CompanyID, HttpContext.Current.Session["ColuumnList"].ToString(), SessionHelper.UserID, SessionUserId, SessionHelper.EnterPriceID, CurrentOptionList);
                    }
   ...
   bool IsFirstCall, ref string message, ref string status, ref bool allSuccesfulRecords, string currentCulture)
        {
...

            {
                ...
                if (LstManufacturerMaster != null && LstManufacturerMaster.Length > 0)
                {
                    ...
                    lst = objImportDAL.GetAllUDFList(SessionHelper.RoomID, ImportMastersDTO.TableName.ManufacturerMaster.ToString());

                    eTurns.DAL.UDFDAL objUDFDAL = new eTurns.DAL.UDFDAL(SessionHelper.EnterPriseDBName);
                    IEnumerable<UDFDTO> UDFDataFromDB = objUDFDAL.GetRequiredUDFsByUDFTableNamePlain("Manufacturers", SessionHelper.RoomID, SessionHelper.CompanyID);
                    ...
                    {
                        lstreturn = objImportDAL.BulkInsert(ImportMastersDTO.TableName.ManufacturerMaster.ToString(), CurrentManufacturerList, SessionHelper.RoomID, SessionHelper.CompanyID, HttpContext.Current.Session["ColuumnList"].ToString(), SessionHelper.UserID, SessionUserId, SessionHelper.EnterPriceID, CurrentOptionList);
                    }
            ...
            bool IsFirstCall, ref string message, ref string status, ref bool allSuccesfulRecords, string currentCulture)
        {
            ...
            //else if (ImportMastersDTO.TableName.TechnicianMaster.ToString() == TableName)
            {
                ...
                if (LstTechnicianMaster != null && LstTechnicianMaster.Length > 0)
                {
                    ...
                    lst = objImportDAL.GetAllUDFList(SessionHelper.RoomID, ImportMastersDTO.TableName.TechnicianMaster.ToString());

                    eTurns.DAL.UDFDAL objUDFDAL = new eTurns.DAL.UDFDAL(SessionHelper.EnterPriseDBName);
                    IEnumerable<UDFDTO> UDFDataFromDB = objUDFDAL.GetRequiredUDFsByUDFTableNamePlain("Technicians", SessionHelper.RoomID, SessionHelper.CompanyID);
                    ...
                    {
                        lstreturn = objImportDAL.BulkInsert(ImportMastersDTO.TableName.TechnicianMaster.ToString(), CurrentTechnicianList, SessionHelper.RoomID, SessionHelper.CompanyID, HttpContext.Current.Session["ColuumnList"].ToString(), SessionHelper.UserID, SessionUserId, SessionHelper.EnterPriceID, CurrentOptionList);
                    }
            ...
            bool IsFirstCall, ref string message, ref string status, ref bool allSuccesfulRecords, string currentCulture)
        {
            ...
            //if (ImportMastersDTO.TableName.ToolCategoryMaster.ToString() == TableName)
            {
                ...
                if (LstToolCategoryMaster != null && LstToolCategoryMaster.Length > 0)
                {
                    ...
                    lst = objImportDAL.GetAllUDFList(SessionHelper.RoomID, ImportMastersDTO.TableName.ToolCategoryMaster.ToString());

                    eTurns.DAL.UDFDAL objUDFDAL = new eTurns.DAL.UDFDAL(SessionHelper.EnterPriseDBName);
                    IEnumerable<UDFDTO> UDFDataFromDB = objUDFDAL.GetRequiredUDFsByUDFTableNamePlain("ToolCategories", SessionHelper.RoomID, SessionHelper.CompanyID);
                    CurrentOptionList = new List<UDFOptionsMain>();
                    ...
                    {
                        lstreturn = objImportDAL.BulkInsert(ImportMastersDTO.TableName.ToolCategoryMaster.ToString(), CurrentToolCategoryList, SessionHelper.RoomID, SessionHelper.CompanyID, HttpContext.Current.Session["ColuumnList"].ToString(), SessionHelper.UserID, SessionUserId, SessionHelper.EnterPriceID, CurrentOptionList);
                    }
            ...
            bool IsFirstCall, ref string message, ref string status, ref bool allSuccesfulRecords, string currentCulture)
        {
...

            {
                ...
                if (LstShipViaMaster != null && LstShipViaMaster.Length > 0)
                {
                    ...
                    lst = objImportDAL.GetAllUDFList(SessionHelper.RoomID, ImportMastersDTO.TableName.ShipViaMaster.ToString());

                    //ShipVias
                    eTurns.DAL.UDFDAL objUDFDAL = new eTurns.DAL.UDFDAL(SessionHelper.EnterPriseDBName);
                    IEnumerable<UDFDTO> UDFDataFromDB = objUDFDAL.GetRequiredUDFsByUDFTableNamePlain("ShipVias", SessionHelper.RoomID, SessionHelper.CompanyID);
                    CurrentOptionList = new List<UDFOptionsMain>();
                    ...
                    {
                        lstreturn = objImportDAL.BulkInsert(ImportMastersDTO.TableName.ShipViaMaster.ToString(), CurrentShipViaList, SessionHelper.RoomID, SessionHelper.CompanyID, HttpContext.Current.Session["ColuumnList"].ToString(), SessionHelper.UserID, SessionUserId, SessionHelper.EnterPriceID, CurrentOptionList);
                    }
            ...
            bool IsFirstCall, ref string message, ref string status, ref bool allSuccesfulRecords)
        {
...

            {
                ...
                if (LstItemManufacturermain != null && LstItemManufacturermain.Length > 0)
                {
                    ...
                    {
                        lstreturn = objImportDAL.BulkInsert(ImportMastersDTO.TableName.ItemManufacturerDetails.ToString(), CurrentItemManufacturerList, SessionHelper.RoomID, SessionHelper.CompanyID, HttpContext.Current.Session["ColuumnList"].ToString(), SessionHelper.UserID, SessionUserId, SessionHelper.EnterPriceID, CurrentOptionList);
                    }
        ...
        bool IsFirstCall, ref string message, ref string status, ref bool allSuccesfulRecords, string currentCulture)
        {
            ...
            //if (ImportMastersDTO.TableName.ItemLocationeVMISetup.ToString() == TableName)
            {
                ...
                {
                    ItemMasterDAL objItemMasterDAL = new ItemMasterDAL(SessionHelper.EnterPriseDBName);
                    lst = objImportDAL.GetAllUDFList(SessionHelper.RoomID, ImportMastersDTO.TableName.BinUDF.ToString());

                    eTurns.DAL.UDFDAL objUDFDAL = new eTurns.DAL.UDFDAL(SessionHelper.EnterPriseDBName);
                    IEnumerable<UDFDTO> UDFDataFromDB = objUDFDAL.GetRequiredUDFsByUDFTableNamePlain("BinUDF", SessionHelper.RoomID, SessionHelper.CompanyID);
                    CurrentOptionList = new List<UDFOptionsMain>();
                    ...
                    foreach (InventoryLocationQuantityMain item in LstInventoryLocation)
                    {
                        ...
                        string errorMsg = string.Empty;
                        CommonUtilityHelper.CheckUDFIsRequired(UDFDataFromDB, item.BinUDF1, item.BinUDF2, item.BinUDF3, item.BinUDF4, item.BinUDF5, out errorMsg, SessionHelper.EnterPriceID, SessionHelper.CompanyID, SessionHelper.RoomID, currentCulture);
                        if (!string.IsNullOrWhiteSpace(errorMsg))
                        {
                        ...
                        if (objDTO.ItemGUID.ToString() != Guid.Empty.ToString() && item.BinNumber.Trim() != "" && saveData)
                        {
                            ...
                            CheckUDF(lst, CurrentOptionList, objDTO.BinUDF1 == null ? "" : objDTO.BinUDF1, CommonUtility.ImportLocationColumn.UDF1.ToString());
                            CheckUDF(lst, CurrentOptionList, objDTO.BinUDF2 == null ? "" : objDTO.BinUDF2, CommonUtility.ImportLocationColumn.UDF2.ToString());
                            CheckUDF(lst, CurrentOptionList, objDTO.BinUDF3 == null ? "" : objDTO.BinUDF3, CommonUtility.ImportLocationColumn.UDF3.ToString());
                            CheckUDF(lst, CurrentOptionList, objDTO.BinUDF4 == null ? "" : objDTO.BinUDF4, CommonUtility.ImportLocationColumn.UDF4.ToString());
                            CheckUDF(lst, CurrentOptionList, objDTO.BinUDF5 == null ? "" : objDTO.BinUDF5, CommonUtility.ImportLocationColumn.UDF5.ToString());
                    ...
                    {
                        lstreturn = objImportDAL.BulkInsert(ImportMastersDTO.TableName.ItemLocationeVMISetup.ToString(), CurrentInventoryLocationQuantityList, SessionHelper.RoomID, SessionHelper.CompanyID, HttpContext.Current.Session["ColuumnList"].ToString(), SessionHelper.UserID, SessionUserId, SessionHelper.EnterPriceID, CurrentOptionList);
                    }
            ...
            bool IsFirstCall, ref string message, ref string status, ref bool allSuccesfulRecords, string currentCulture)
        {
...

            {
                ...
                if (LstCategoryMaster != null && LstCategoryMaster.Length > 0)
                {
                    ...
                    lst = objImportDAL.GetAllUDFList(SessionHelper.RoomID, ImportMastersDTO.TableName.InventoryClassificationMaster.ToString());


                    eTurns.DAL.UDFDAL objUDFDAL = new eTurns.DAL.UDFDAL(SessionHelper.EnterPriseDBName);
                    IEnumerable<UDFDTO> UDFDataFromDB = objUDFDAL.GetRequiredUDFsByUDFTableNamePlain("InventoryClassification", SessionHelper.RoomID, SessionHelper.CompanyID);
                    ...
                    if (CurrentInventoryClassificationList.Count > 0)
                    {
                        ...
                        lstreturn = objImportDAL.BulkInsert(ImportMastersDTO.TableName.InventoryClassificationMaster.ToString(), CurrentInventoryClassificationList, SessionHelper.RoomID, SessionHelper.CompanyID, HttpContext.Current.Session["ColuumnList"].ToString(), SessionHelper.UserID, SessionUserId, SessionHelper.EnterPriceID, CurrentOptionList);
                    }
            ...
            bool IsFirstCall, ref string message, ref string status, ref bool allSuccesfulRecords)
        {
...

            {
                ...
                if (LstGLAccountMaster != null && LstGLAccountMaster.Length > 0)
                {
                    ...
                    lst = objImportDAL.GetAllUDFList(SessionHelper.RoomID, ImportMastersDTO.TableName.GLAccountMaster.ToString());
                    ...
                    {
                        lstreturn = objImportDAL.BulkInsert(ImportMastersDTO.TableName.GLAccountMaster.ToString(), CurrentGLAccountList, SessionHelper.RoomID, SessionHelper.CompanyID, HttpContext.Current.Session["ColuumnList"].ToString(), SessionHelper.UserID, SessionUserId, SessionHelper.EnterPriceID, CurrentOptionList);
                    }
        ...
        bool IsFirstCall, ref string message, ref string status, ref bool allSuccesfulRecords)
        {
...

            {
                ...
                if (LstItemManufacturermain != null && LstItemManufacturermain.Length > 0)
                {
                        ...
                        if ((item.ItemNumber != null && item.ModuleName != string.Empty && item.BarcodeString != string.Empty))
                        {
                            ...
                            else if (item.ModuleName.ToLower().Trim() == "assets")
                            {
                                ...
                                if (objAssetmasterDTO != null)
                                {
                                    ...
                                    AssetMasterDTO objAssetMasterDTO = objAssetmasterDAL.GetAssetsByName(item.ItemNumber, SessionHelper.RoomID, SessionHelper.CompanyID).FirstOrDefault();
                                    if (objAssetMasterDTO != null)
                                    {
                            ...
                            if (ItemGUID.HasValue && ItemGUID != Guid.Empty)
                            {
                                ...
                                objBarcodeMasterDTO = objBarcodeMasterDAL.GetBarcodeByBarcodeStringRefGUID(SessionHelper.RoomID, SessionHelper.CompanyID, item.BarcodeString, Convert.ToString(ItemGUID.Value)).FirstOrDefault();
                            }
                            else
                            {
                    ...
                    {
                        lstreturn = objImportDAL.BulkInsert(ImportMastersDTO.TableName.BarcodeMaster.ToString(), CurrentBarcodeList, SessionHelper.RoomID, SessionHelper.CompanyID, HttpContext.Current.Session["ColuumnList"].ToString(), SessionHelper.UserID, SessionUserId, SessionHelper.EnterPriceID, CurrentOptionList);
                    }
        ...
        bool IsFirstCall, ref string message, ref string status, ref bool allSuccesfulRecords, string currentCulture)
        {
...

            {
                ...
                if (LstCategoryMaster != null && LstCategoryMaster.Length > 0)
                {
                    ...
                    lst = objImportDAL.GetAllUDFList(SessionHelper.RoomID, ImportMastersDTO.TableName.CategoryMaster.ToString());
                    eTurns.DAL.UDFDAL objUDFDAL = new eTurns.DAL.UDFDAL(SessionHelper.EnterPriseDBName);
                    IEnumerable<UDFDTO> UDFDataFromDB = objUDFDAL.GetRequiredUDFsByUDFTableNamePlain("CategoryMaster", SessionHelper.RoomID, SessionHelper.CompanyID);
                    CurrentOptionList = new List<UDFOptionsMain>();
                    ...
                    {
                        lstreturn = objImportDAL.BulkInsert(ImportMastersDTO.TableName.CategoryMaster.ToString(), CurrentCategoryList, SessionHelper.RoomID, SessionHelper.CompanyID, HttpContext.Current.Session["ColuumnList"].ToString(), SessionHelper.UserID, SessionUserId, SessionHelper.EnterPriceID, CurrentOptionList);
                    }
                    if (CurrentBlankCategoryList.Count > 0)
                    {
            ...
            bool IsFirstCall, ref string message, ref string status, ref bool allSuccesfulRecords, string currentCulture)
        {
...

            {
                ...
                if (LstCategoryMaster != null && LstCategoryMaster.Length > 0)
                {
                    ...
                    lst = objImportDAL.GetAllUDFList(SessionHelper.RoomID, ImportMastersDTO.TableName.CostUOMMaster.ToString());

                    eTurns.DAL.UDFDAL objUDFDAL = new eTurns.DAL.UDFDAL(SessionHelper.EnterPriseDBName);
                    IEnumerable<UDFDTO> UDFDataFromDB = objUDFDAL.GetRequiredUDFsByUDFTableNamePlain("CostUOM", SessionHelper.RoomID, SessionHelper.CompanyID);
                    CurrentOptionList = new List<UDFOptionsMain>();
                    ...
                    {
                        lstreturn = objImportDAL.BulkInsert(ImportMastersDTO.TableName.CostUOMMaster.ToString(), CurrentCostUOMList, SessionHelper.RoomID, SessionHelper.CompanyID, HttpContext.Current.Session["ColuumnList"].ToString(), SessionHelper.UserID, SessionUserId, SessionHelper.EnterPriceID, CurrentOptionList);
                    }
                    if (CurrentBlankCategoryList.Count > 0)
                    {
            ...
            bool IsFirstCall, ref string message, ref string status, ref bool allSuccesfulRecords)
        {
            ...
            //else if (ImportMastersDTO.TableName.CustomerMaster.ToString() == TableName)
            {
                ...
                if (LstCustomerMaster != null && LstCustomerMaster.Length > 0)
                {
                    ...
                    lst = objImportDAL.GetAllUDFList(SessionHelper.RoomID, ImportMastersDTO.TableName.CustomerMaster.ToString());
                    CurrentOptionList = new List<UDFOptionsMain>();
                    ...
                    {
                        lstreturn = objImportDAL.BulkInsert(ImportMastersDTO.TableName.CustomerMaster.ToString(), CurrentCustomerList, SessionHelper.RoomID, SessionHelper.CompanyID,
        ...
        , ref bool allSuccesfulRecords)
        {
            ...
            //else if (ImportMastersDTO.TableName.SupplierMaster.ToString() == TableName)
            {
                ...
                if (LstSupplierMaster != null && LstSupplierMaster.Length > 0)
                {
                    ...
                    lst = objImportDAL.GetAllUDFList(SessionHelper.RoomID, ImportMastersDTO.TableName.SupplierMaster.ToString());
                    ...
                    foreach (SupplierMasterMain item in LstSupplierMaster)
                    {
                        ...
                        if (string.IsNullOrWhiteSpace(errors) && item.SupplierName.Trim() != "" && item.Contact.Trim() != "" && item.Phone.Trim() != "")
                        {
                            ...
                            CheckUDF(lst, CurrentOptionList, objDTO.UDF6 == null ? "" : objDTO.UDF6, CommonUtility.ImportSupplierColumn.UDF6.ToString());
                            CheckUDF(lst, CurrentOptionList, objDTO.UDF7 == null ? "" : objDTO.UDF7, CommonUtility.ImportSupplierColumn.UDF7.ToString());
                            CheckUDF(lst, CurrentOptionList, objDTO.UDF8 == null ? "" : objDTO.UDF8, CommonUtility.ImportSupplierColumn.UDF8.ToString());
                            CheckUDF(lst, CurrentOptionList, objDTO.UDF9 == null ? "" : objDTO.UDF9, CommonUtility.ImportSupplierColumn.UDF9.ToString());
                            CheckUDF(lst, CurrentOptionList, objDTO.UDF10 == null ? "" : objDTO.UDF10, CommonUtility.ImportSupplierColumn.UDF10.ToString());
                            //CheckUDF(lst, CurrentOptionList, objDTO.UDF6 == null ? "" : objDTO.UDF6, CommonUtility.ImportSupplierColumn.UDF6.ToString());
                    ...
                    if (CurrentSupplierList.Count > 0)
                    {
                        ...
                        //lstreturn = objImport.BulkInsert(ImportMastersDTO.TableName.SupplierMaster.ToString(), CurrentSupplierList, SessionHelper.RoomID, SessionHelper.CompanyID, Session["ColuumnList"].ToString(), SessionHelper.UserID, CurrentOptionList);
                        lstreturn = objImportDAL.BulkInsertWithChiled(ImportMastersDTO.TableName.SupplierMaster.ToString(), CurrentSupplierList, SessionHelper.RoomID, SessionHelper.CompanyID,
        ...
        public void SaveItemMaster(List<UDFOptionsCheckDTO> lst, List<UDFOptionsMain> CurrentOptionList, List<Guid> lstItemGUID,
            ...
            , ref bool allSuccesfulRecords, string currentCulture
            )
        {
            {
                long SessionUserId = SessionHelper.UserID;
                ...
                if (LstItemMaster != null && LstItemMaster.Length > 0)
                {
                    ...
                    lst = objImportDAL.GetAllUDFList(SessionHelper.RoomID, ImportMastersDTO.TableName.ItemMaster.ToString());

                    eTurns.DAL.UDFDAL objUDFDAL = new eTurns.DAL.UDFDAL(SessionHelper.EnterPriseDBName);
                    IEnumerable<UDFDTO> UDFDataFromDB = objUDFDAL.GetRequiredUDFsByUDFTableNamePlain("ItemMaster", SessionHelper.RoomID, SessionHelper.CompanyID);
                    ...
                    string columnList = "ID,RoomName,IsAllowOrderCostuom,ReplenishmentType,GlobMarkupParts,GlobMarkupLabor,IsELabel";
                    ...
                    List<ItemMasterDTO> lstItemMaster = itemMasterDAL.GetAllItemsWithJoins(SessionHelper.RoomID, SessionHelper.CompanyID, false, false, string.Empty).ToList();
                    //bool IsEmailPOInBody = false; bool IsEmailPOInPDF = false; bool IsEmailPOInCSV = false; bool IsEmailPOInX12 = false;
                    foreach (BOMItemMasterMain item in LstItemMaster)
                    {
                        ...
                        {
                            objDTO.DefaultLocation = objImportDAL.GetBinForImportItem(item.InventryLocation, objDTO.ItemNumber, SessionHelper.RoomID, SessionHelper.CompanyID, SessionHelper.UserID); // GetIDs(ImportMastersDTO.TableName.BinMaster, item.InventryLocation);
                        }
                        ...
                        objDTO.eLabelKey = item.eLabelKey;
                        ...
                        /*/////CODE FOR CHECK UDF IS REQUIRED///////*/
                        string errorMsg = string.Empty;
                        CommonUtilityHelper.CheckUDFIsRequired(UDFDataFromDB, item.UDF1, item.UDF2, item.UDF3, item.UDF4, item.UDF5, out errorMsg, SessionHelper.EnterPriceID, SessionHelper.CompanyID, SessionHelper.RoomID, currentCulture, null, item.UDF6, item.UDF7, item.UDF8, item.UDF9, item.UDF10);
                    ...
                    if (CurrentItemList.Count > 0)
                    {
                        ...
                        lstreturn = objImportDAL.BulkInsert(ImportMastersDTO.TableName.ItemMaster.ToString(), CurrentItemList, SessionHelper.RoomID, SessionHelper.CompanyID, HttpContext.Current.Session["ColuumnList"].ToString(), SessionHelper.UserID, SessionUserId, SessionHelper.EnterPriceID, CurrentOptionList, isImgZipAvail, isLink2ZipAvail);
                    }
                    ...
                    #region Update Solum details
                    if (objRoomDTO.IsELabel)
                    {
                        if (lstreturn.Count() > 0 && lstreturn.Where(l => l.Status != null && l.Status.ToLower() == "success").ToList().Count() > 0)
                        {
                            foreach (var item in lstreturn)
                            {
                                if (item.SupplierPartNo != String.Empty && item.SupplierPartNo != null)
                                {
                                    ...
                                    string DefaultLocationName = BinMasterDAL.GetBinByID(Int64.Parse(Convert.ToString(item.DefaultLocation)), SessionHelper.RoomID, SessionHelper.CompanyID).BinNumber;
                                    itemMasterDAL.AddUpdateSolumnProduct(item.SupplierPartNo, item.ItemNumber, item.GUID, item.Description, item.MinimumQuantity.ToString(), item.MaximumQuantity.ToString(), item.DefaultReorderQuantity, item.CostUOMID, DefaultLocationName, item.OnOrderQuantity, string.Empty, SessionHelper.EnterPriceID, SessionHelper.CompanyID, SessionHelper.RoomID, item.eLabelKey);
                                }
                            }
                        }
                    } 
        ...
        public static Int64 GetIDs(ImportMastersDTO.TableName TableName, string strVal, long longID = 0)
        {
            Int64 returnID = 0;
        ...
        public static void CheckUDF(List<UDFOptionsCheckDTO> lst, List<UDFOptionsMain> CurrentOptionList, string objDTOUDF, string UDFs)
        {
        ...
        public static Guid GetGUID(ImportMastersDTO.TableName TableName, string strVal, string optValue = "", QuickListType QLType = QuickListType.General)
        {
        ...
        public static bool CheckBinStatus(string BinNumber)
        {
            bool result = false;
            try
            {
