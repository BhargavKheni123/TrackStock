...
$(document).ready(function () {
    ...
    $('#btnAllRecordDownload').click(function () {
        ...
        $('#btnAllRecordDownload').attr("disabled", "disabled");
        $('#DivLoading').show();
    ...
    $('#btnSave').click(function () {
        ...
        if (selectedModuleVal == BinMasterselval) {
            ...
            $('#myImportDataTable tbody tr').each(function () {
                ...
                var tmpUNSPSC = $(this).find('#aUNSPSC_' + tempItemID).val();
                var tmpLongDescription = $(this).find('#aLongDescription_' + tempItemID).val();
                var tmpCategory = $(this).find('#aCategory_' + tempItemID).val();
                RowNumber++;
                var data = {
                    "ID": RowNumber,
                    "ItemNumber": tempItemNumber,
                    ...
                    "UNSPSC": tmpUNSPSC,
                    "LongDescription": tmpLongDescription,
...
function ImportAjaxCall(_arrItems, IsFirstCall, isLoadAllItems) {
    ...
    $.ajax({
        url: SaveImportURL,
        type: 'POST',
        data: { 'TableName': CurModuleval, 'para': JSON.stringify(DataBatch), 'HasMoreRecords': true, 'IsFirstCall': IsFirstCall, 'TotalCount': TotalCount, 'isImgZipAvail': isImgZipAvail, 'isLink2ZipAvail': isLink2ZipAvail },
        cache: false,
        timeout: 0,
...
function CheckZIPFileName() {
    if ($('#ddlModule option:selected').attr('value') == ItemMasterselval) {
...
function isUniqueItemAndSupplierNumber() {
    ...
    $('#myImportDataTable tbody tr').each(function () {
        ...
        var supplierName = $(this).find('input[id^="aSupplierName_"]').val();
        var isavailable = existing.filter(function (item) {
            return item.suppliernumber == suppliernumber && item.supplierName == supplierName && item.itemnumber === itemnumber
        });
        ...
        i++;
        existing.push({ suppliernumber, itemnumber, supplierName });
    });
    return isError;
}
