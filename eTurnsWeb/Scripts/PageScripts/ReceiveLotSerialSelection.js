...
function PrepareReceiveDataTable(objTransferItemDTO) {
    ...
    var Curtable = $('#' + objTransferItemDTO.tableID).dataTable({
        ...
        "fnServerData": function (sSource, aoData, fnCallback, oSettings) {
            ...
            aoData.push({ "name": "TransferDetailGUID", "value": objTransferItemDTO.TransferDetailGUID });
            ...
            if (objTransferItemDTO.ItemGUID != '00000000-0000-0000-0000-000000000000' && objTransferItemDTO.ItemGUID != '')
                aoData.push({ "name": "TransferQuantity", "value": FormatedCostQtyValues($("#txtReceiveQuantity_" + objTransferItemDTO.ItemGUID + "_" + objTransferItemDTO.RequisitionDetailGUID).val(), 2) });
            else
                aoData.push({ "name": "TransferQuantity", "value": FormatedCostQtyValues($("#txtReceiveQuantity_" + objTransferItemDTO.ToolGUID + "_" + objTransferItemDTO.RequisitionDetailGUID).val(), 2) });
...
$(document).ready(function () {
    ...
    if (rights == "ViewOnly") {
        $("input[type='text'][name^='txtReceiveQuantity_']").keypress(function () {
            return false;
        });
    }
    ...
    $("#DivTransferSelection").on('change', "input[type='text'][name^='txtLotOrSerailNumber']", function (e) {
        ...
        var ids = $(this).parent().parent().parent().parent().parent().parent().parent().parent().parent().find("[id^='hdnTransferIds_']").val().split('_');
        var aPos = $("#tblItemReceive_" + ids[1].toString() + "_" + ids[2].toString()).dataTable().fnGetPosition($(this).parent().parent().parent()[0]);
        var aData = $("#tblItemReceive_" + ids[1].toString() + "_" + ids[2].toString()).dataTable().fnGetData(aPos);

        var dtThisItem = $("#tblItemReceive_" + ids[1].toString() + "_" + ids[2].toString()).dataTable();
        ...
        $("#tblItemReceive_" + ids[1].toString() + "_" + ids[2].toString() + " tbody tr").each(function (i) {
            if (i != row_id) {
                var tr = $(this);
        ...
        if (isDuplicateEntry == true) {
            ...
            $.ajax({
                ...
                dataType: 'json',
                data: "{ ItemGuid: '" + ids[1].toString() + "', SerialOrLotNumber: '" + $.trim($(objCurtxt).val()) + "',BinID: '" + aData.BinID + "', TransferDetailGUID: '" + ids[2].toString() + "'}",
                success: function (RetData) {
                    if (RetData.ID > 0) {
                        IsCheckViewRight = false;

                        var spnPoolQuantity = parseFloat($("#txtReceiveQuantity_" + ids[1].toString() + "_" + ids[2].toString()).val());
    ...
    $("#DivTransferSelection").on('focus', "input[type='text'][name^='txtLotOrSerailNumber']", function (e) {
        ...
        var aPos = $("#tblItemReceive_" + ids[1].toString() + "_" + ids[2].toString()).dataTable().fnGetPosition($(this).parent().parent().parent()[0]);
        var aData = $("#tblItemReceive_" + ids[1].toString() + "_" + ids[2].toString()).dataTable().fnGetData(aPos);
        var dtItemPull = "#tblItemReceive_" + ids[1].toString() + "_" + ids[2].toString();
        var strSerialLotNos = "";
        ...
        if ($(this).hasClass("AutoSerialLot")) {
            $(this).autocomplete({
                source: function (request, response) {
                    $.ajax({
                        url: '/Transfer/GetLotOrSerailNumberListForReceiveTransfer',
                        contentType: 'application/json',
                        dataType: 'json',
                        data: {
                            maxRows: 1000,
                            ...
                            transferDetailGUID: ids[2].toString(),
                            BinID: aData.BinID,
    ...
    $("#DivTransferSelection").on("change", "input[type='text'][name='txtTransferQty']", function () {
        var ids = $(this).parent().parent().parent().parent().parent().parent().parent().parent().find("[id^='hdnTransferIds_']").val().split('_');
        var aPos = $("#tblItemReceive_" + ids[1].toString() + "_" + ids[2].toString()).dataTable().fnGetPosition($(this).parent().parent()[0]);
        $("#tblItemReceive_" + ids[1].toString() + "_" + ids[2].toString()).dataTable().fnGetData(aPos).PullQuantity = $(this).val();
    });
    ...
    $("#DivTransferSelection").on("click", "input[type='button'][name='btnLoadMoreLots']", function () {
        ...
        var dtID = "#tblItemReceive_" + vItemGUID + "_" + vRequisitionDetailGUID;
        var strIds = "";

        var MaxQuantity = $("#txtReceiveQuantity_" + vItemGUID + "_" + vRequisitionDetailGUID)[0].value;
        var TotalQuantity = 0;
        $("#tblItemReceive_" + vItemGUID + "_" + vRequisitionDetailGUID).find("[id*='txtTransferQty_']").each(function () {
    ...
    $("#DivTransferSelection").off("click", "input[type='button'][name='btnReceivePopup']");
    $("#DivTransferSelection").on("click", "input[type='button'][name='btnReceivePopup']", function () {
        $('#DivLoading').show();
        var errorMsg = '';
        var iserror = 0;
        var ErrorSrNo = 0;
        ...
        if (ErrorMessage == "") {
            ...
            var ID = vItemGUID;
            var SpanQty = $("#DivTransferSelection").find("#txtReceiveQuantity_" + vItemGUID + "_" + vRequisitionDetailGUID);
            var dt = $("#tblItemReceive_" + vItemGUID + "_" + vRequisitionDetailGUID).dataTable();
            var currentData = dt.fnGetData();
            var strtransferobj = JSON.parse($("#hdnReceiveMasterDTO_" + vItemGUID + "_" + vRequisitionDetailGUID).val());
            $("#tblItemReceive_" + vItemGUID + "_" + vRequisitionDetailGUID).find("tbody").find("tr").each(function (index, tr) {                
                var txtTransferQty = $(tr).find("input[type='text'][name='txtTransferQty']").val();
                var hdnLotNumberTracking = $(tr).find("input[name='hdnLotNumberTracking']").val();
                var hdnSerialNumberTracking = $(tr).find("input[name='hdnSerialNumberTracking']").val();
                var hdnDateCodeTracking = $(tr).find("input[name='hdnDateCodeTracking']").val();
                ...
                if (txtTransferQty != "") {
                    ...
                    var obj = {
                        "LotOrSerailNumber": txtLotOrSerailNumber, "BinNumber": hdnBinNumber, "PullQuantity": parseFloat(txtTransferQty.toString())
                                    ...
                                    , "ItemGUID": strtransferobj.ItemGUID, "BinID": strtransferobj.Bin, "ID": strtransferobj.Bin
                    };
            ...
            if (arrItemDetails.length > 0) {                
                ...
                var obj = { 'ItemID': strtransferobj.ItemGUID, 'DetailID': strtransferobj.GUID, 'ReceiveBinName': strtransferobj.ItemDetail.BinNumber, 'IsStaging': strtransferobj.IsStaging, 'QtyToReceive': qty, 'ItemLocationDetails': arrItemDetails };

                $.ajax({
                    url: 'ReceiveItemQuantity',
                    ...
                    success: function (response) {
                        if (response.Status === true) {
                            ...
                            errorMsg = errorMsg + '<b style="color: Green;">' + ErrorSrNo + ') ' + strtransferobj.ItemNumber + ': Received Successfully. </b> <br />';
                        }
                        else {
                            iserror = 1;
                            ErrorSrNo = parseInt(ErrorSrNo) + 1;
                            errorMsg = errorMsg + '<b style="color: Olive;">' + ErrorSrNo + ') ' + strtransferobj.ItemNumber + ': ' + response.Message + ' </b> <br />';
                    ...
                    error: function (xhr) {
                        iserror = 1;
                        ErrorSrNo = parseInt(ErrorSrNo) + 1;
                        errorMsg = errorMsg + '<b style="color: Olive;">' + ErrorSrNo + ') ' + strtransferobj.ItemNumber + ': Internal Server Error. </b> <br />';
                    }
                });
                setTimeout(function () {
                    $('#DivLoading').hide();
                    if (iserror >= 1) {
                        ...
                        closeMessageModalInPopup();
                        errorMsg = '<b>' + MsgReceivedSuccessfully +'</b><br />'
                        $('#TrnReceivedInfoDialog').find("#TrnReceivedMSG").html(errorMsg);
                        $('#TrnReceivedInfoDialog').modal();
                        $("#DivTransferSelection").empty();
                        $('#DivTransferSelection').dialog('close');
    ...
    $("#DivTransferSelection").off("click", "input[type='button'][name='btnReceiveAllPopUp']");
    $("#DivTransferSelection").on("click", "input[type='button'][name='btnReceiveAllPopUp']", function () {
        TransferAllNewFlow();
    });

    $("#DivTransferSelection").off("click", "input[type='button'][name='btnCancelReceivePopup']");
    $("#DivTransferSelection").on("click", "input[type='button'][name='btnCancelReceivePopup']", function () {
        $("#DivTransferSelection").empty();
        $('#DivTransferSelection').dialog('close');
    });
...
function ValidateSingleTransfer(vItemGUID, RequisitionDetailGUID) {
    ...
    var SpanQty = $("#DivTransferSelection").find("#txtReceiveQuantity_" + vItemGUID + "_" + RequisitionDetailGUID);

    var TotalEntered = 0;
    $("#tblItemReceive_" + vItemGUID + "_" + RequisitionDetailGUID).find("tbody").find("tr").each(function (index, tr) {
        var txtTransferQty = $(tr).find("input[type='text'][name='txtTransferQty']").val();
    ...
    if (isMoreQty == false) {
        ...
        if (TotalEntered != transferQty) {
            errormsg = errormsg + "\n" + MsgRecieveEnterValidation.replace("{0}", TotalEntered).replace("{1}", transferQty);
        }
    }
    else {
        errormsg = errormsg = "\n" + MsgRecieveQtyValidation;
...
function TransferAllNewFlow() {
    ...
    if (ErrorMessage == "") {        
        $("#DivTransferSelection").find("table[id^='tblItemReceiveheader']").each(function (indx, tblHeader) {
            var strtransferobj = JSON.parse($(tblHeader).find("input[name='hdnReceiveMasterDTO']").val());
            arrItemDetails = new Array();
            ...
            if (ID != '00000000-0000-0000-0000-000000000000')
                SpanQty = $(tblHeader).find("#txtReceiveQuantity_" + ID + "_" + RequisitionDetailGUID);
            else
                SpanQty = $(tblHeader).find("#txtReceiveQuantity_" + vToolGUID + "_" + RequisitionDetailGUID);

            var dt = null;
            if ($("#tblItemReceive_" + ID + "_" + RequisitionDetailGUID).length > 0)
                dt = $("#tblItemReceive_" + ID + "_" + RequisitionDetailGUID).dataTable();

            if ($("#tblItemReceive_" + ID + "_" + RequisitionDetailGUID).length > 0) {
                var currentData = dt.fnGetData();
                $("#tblItemReceive_" + ID + "_" + RequisitionDetailGUID).find("tbody").find("tr").each(function (index, tr) {
                    var txtTransferQty = $(tr).find("input[type='text'][name='txtTransferQty']").val();
                    var hdnLotNumberTracking = $(tr).find("input[name='hdnLotNumberTracking']").val();
                    var hdnSerialNumberTracking = $(tr).find("input[name='hdnSerialNumberTracking']").val();
                    var hdnDateCodeTracking = $(tr).find("input[name='hdnDateCodeTracking']").val();
        ...
        setTimeout(function () {
            ...
            if (iserror >= 1) {
                ...
                $("#DivTransferSelection").empty();
                $('#DivTransferSelection').dialog('close');
                FillTransferDetailGrid(); //window.parent.$('#aEditLink').trigger('click'); 
            }
            else {
                closeMessageModalInPopup();
                errorMsg = '<b>' + MsgReceivedSuccessfully +'</b><br />'
...
function ValidateAllTransfer() {
    ...
    $("#DivTransferSelection").find("table[id^='tblItemReceiveheader']").each(function (indx, tblHeader) {
        var ID = $(tblHeader).prop("id").split('_')[1];
        var RequisitionDetailGUID = $(tblHeader).prop("id").split('_')[2];
        var SpanQty = $(tblHeader).find("#txtReceiveQuantity_" + ID + "_" + RequisitionDetailGUID);
        ...
        if ($("#tblItemReceive_" + ID + "_" + RequisitionDetailGUID).length > 0) {
            ...
            if (isMoreQty == false) {
                ...
                errormsg = MsgRecieveQtyValidation;
            }
        }
    });
