...
$(document).ready(function () {
    setTimeout(function () {
        ...
        $("#StagingLocationbinreplanish").on({
            focus: function () {
                console.log("called");
        ...
        $("#StagingLocationbinreplanish").on({ click: function () { $(this).siblings('#txtLocation').trigger("focus"); } }, ".ShowAllOptionsBin");
    ...
    $("#IsAllowOrderCostuom").change(function (e) {
        ...
        if ($(this).attr('checked') == 'checked') {
            ...
            $('#ItemStagingLocationLevelQuanity tbody tr').each(function () {
...
function CheckBeforeSave() {
    ...
    if (parseInt(itemt) != 4) {
        ...
        var TempLocationrows1 = $("#ItemStagingLocationLevelQuanity").dataTable().fnGetNodes();
    ...
    $.ajax({
        ...
        success: function (responseHistory) {
            ...
            if (responseHistory.errorMessage == "") {
                ...
                $('#HistoryInfoDialog').find("#HistoryMSG").html("BinHistory Error: " + responseHistory.errorMessage);
                closeHistoryInfoModel();
                $('#HistoryInfoDialog').modal();
            }
        }
...
function readURL(input) {
    ...
    if (input.files && input.files[0]) {
        ...
        if (isError == true) {
            ...
            $("#spanGlobalMessage").removeClass('succesIcon WarningIcon').addClass('errorIcon');
            $("#spanGlobalMessage").text('Please select correct file name.');
            $('div#target').fadeToggle();
    ...
    if (strURL != '' && strURL != null) {
        $("<img>", {
            ...
            error: function () {
                ...
                $("#spanGlobalMessage").removeClass('succesIcon WarningIcon').addClass('errorIcon');
                $("#spanGlobalMessage").text('Invalid URL. please enter valid URL');
                $('div#target').fadeToggle();
                //$("div#target").delay(5000).fadeOut(200);
                $("div#target").delay(DelayTime).fadeOut(FadeOutTime);
    ...
    if (strURL != '' && strURL != null) {
        $("<img>", {
            ...
            error: function () {
                //alert('Invalid URL. please enter valid URL');
                $("#spanGlobalMessage").removeClass('succesIcon WarningIcon').addClass('errorIcon');
                $("#spanGlobalMessage").text('Invalid URL. please enter valid URL for Link2');
                $('div#target').fadeToggle();
                //$("div#target").delay(5000).fadeOut(200);
                $("div#target").delay(DelayTime).fadeOut(FadeOutTime);
...
function EnableDisableLabour(IsDisable) {
    ...
    if (IsDisable) {
        ...
        $('#liStagingLocationListInline').hide();
    }
    else {
        $('#ulSupplierDetails').show();
        $('#liStagingLocationListInline').show();
    }
}
...
function BindBinReplanishStaging(obj) {
    $('#divStagingBinReplanish').empty();
    $('#DivLoading').show();
    $.get(Inventory_LoadStagingLocationsOfItem + '?ItemGUID=' + ItemGUID + '&AddCount=0', function (data) {
        $('#divStagingBinReplanish').html(data);
...
function AddNewStagingBinReplanish() {
    RemoveLeadingTrailingSpace("StagingLocationbinreplanish");
...
function SavetoSeesionBinReplanish(obj1) {
    ...
    var TempSuprows = $("#ItemStagingLocationLevelQuanity").dataTable().fnGetNodes();
    var iCountSup = 0;
    ...
    if (vlocIsEDPQ == true && parseFloat(vDPQty) <= 0) {
        $("#spanGlobalMessage").removeClass('succesIcon WarningIcon').addClass('errorIcon');
        $("#spanGlobalMessage").text('Default Pull Quantity must be greater than Zero.');
        $('div#target').fadeToggle();
        //$("div#target").delay(2000).fadeOut(200);
        $("div#target").delay(DelayTime).fadeOut(FadeOutTime);
    ...
    if (vlocIsEDRQ == true && parseFloat(vDRQty) <= 0) {
        $("#spanGlobalMessage").removeClass('succesIcon WarningIcon').addClass('errorIcon');
        $("#spanGlobalMessage").text('Default Reorder Quantity must be greater than Zero.');
        $('div#target').fadeToggle();
        //$("div#target").delay(2000).fadeOut(200);
        $("div#target").delay(DelayTime).fadeOut(FadeOutTime);
...
function SavetoSeesionItemLocationAll() {
    //
    if ($("#ItemStagingLocationLevelQuanity").length > 0) {
        var TempSuprows3 = $("#ItemStagingLocationLevelQuanity").dataTable().fnGetNodes();
        var iCountSup = 0;
...
function DeletetoSeesionStagingBinReplanishSingle(obj) {
    var vardelete = false;
    ...
    if (SavetoSeesionItemLocationAllNew(vhdnGUID)) {
        ...
        if (vhdnGUID == '00000000-0000-0000-0000-000000000000') {
            //client side remove
            $(obj).parent().parent().remove();
            BindBinReplanishStaging(vardelete);
        }
        else {
            if (vItemguid == '') {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (response.status == 'referencecount') {
                        $("#spanGlobalMessage").removeClass('succesIcon errorIcon').addClass('WarningIcon');
                        $("#spanGlobalMessage").text('ItemLocation ' + response.ErrorMessage + ' have quantity(ies). So, not able to delete it.');
                        $('div#target').fadeToggle();
                        //$("div#target").delay(2000).fadeOut(200);
                        $("div#target").delay(DelayTime).fadeOut(FadeOutTime);
                    }
...
function is_valid_url(curobj) {
    ...
    if (url != '' && url != null) {
        ...
        if (IsValidURL == false) {
            ...
            $("#spanGlobalMessage").removeClass('succesIcon WarningIcon').addClass('errorIcon');
            $("#spanGlobalMessage").text('Invalid URL, kindly enter valid URL');
            $('div#target').fadeToggle();
            //$("div#target").delay(5000).fadeOut(200);
            $("div#target").delay(DelayTime).fadeOut(FadeOutTime);
...
function AllowOhQty() {
    ...
    setTimeout(function () {
        ...
        if (ItemID == 0) {
            ...
            if (!srtrck && !lottrck && !datecodetrck && (isBuildBreak !== undefined && isBuildBreak.is(":checked") === false)) {
                $("input#OnHandQuantity").removeProp("readonly").removeClass("disableBack");
