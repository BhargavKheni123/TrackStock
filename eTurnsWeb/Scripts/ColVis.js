    ...
    ColVis.prototype = {
            ...
            for (var i = 0, iLen = aoColumns.length; i < iLen; i++) {
                if (this.dom.buttons[i] !== null) {
                    if (aoColumns[i].bVisible) {
                        ...
                        //Below code Added by CP on 03-Sep-2014
                        if ($(aoColumns[i].nTh).hasClass('NotHide'))
                            $('input', this.dom.buttons[i]).attr('disabled', 'disabled');

                        if ($(aoColumns[i].nTh).hasClass('NotSortable')) {
                            aoColumns[i].bSortable = false;
                            aoColumns[i].sSortingClass = "";
                            aoColumns[i].sSortingClassJUI = "";
                            $(aoColumns[i].nTh.childNodes[0]).find('span.DataTables_sort_icon').css('display', 'none')
                        }
                        //Above code Added by CP on 03-Sep-2014
                    }
                    else {
        ...
        */
        "_fnDomColumnButton": function (i) {
			...
			nSpan = document.createElement('span'),
			dt = this.s.dt;
            nButton.id = isNaN(GetColumnIndex(oColumn.sTitle.trim())) ? 0 : GetColumnIndex(oColumn.sTitle.trim());
            ...
            $(nSpan).html(
			'<span class="ColVis_radio"><input id="' + nButton.id + '_" type="checkbox"/></span>' +
            ...
            $(nButton).click(function (e) {
                ...
                //Below code Added by CP on 03-Sep-2014
                if ($(oColumn.nTh).hasClass('NotHide') && !showHide)
                    return false;
                //Above code Added by CP on 03-Sep-2014
