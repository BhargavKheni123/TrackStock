...
$('#btnReceiveAllNew').on('click', function () {
    ...
    if (!isNaN(ItemSelected.length) && ItemSelected.length > 0) {
        ...
        for (var i = 0; i < ItemSelected.length; i++) {
            ...
            else if (gblActionName.toLowerCase() == "orderlist" && $.trim($(ItemSelected[i]).find('#hdnIsCloseItem').val()) == 'True') {
                isClosedItem = true;
            }
        ...
        $("#spanGlobalMessage").html(MsgSelectRowToReceive);
...
function AddRowLotExpPreRcv (objthis)
{
    ...
    var ClassApplayIndex = $('#tblPreReceiveHeader tbody tr').length;
    if (ClassApplayIndex % 2 == 0) {
        $(lsttrNew).removeClass("odd");
        $(lsttrNew).addClass("even");
    }
    else {
        $(lsttrNew).removeClass("even");
        $(lsttrNew).addClass("odd");
    }

    var ind = parseInt($(objthis).parent().parent().parent().children().index($(objthis).parent().parent())) + 2;
...
function SaveLargeReceiveData(objReceiveArray) {
    ...
    for (var k = 0; k < objReceiveArray.length; k++) {
        $.ajax({
            "url": "/Receive/SaveReceiveInformation",
    ...
    if (errorMsgOuter.length > 0) {
        errorMsgOuter = "<b>" + ResReceiveOrderDetails +"</b><br/>" + errorMsgOuter;
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                if (errorMsg.length > 0) {
                    errorMsg = "<b>" + ResReceiveOrderDetails +"</b><br/>" + errorMsg;
...
function OpenPreReceiveInfoPage(objToFillPreReceive) {
    var ajaxurl = '/Receive/FillPreReceiveInformation';

    if (objToFillPreReceive.SerialNumberTracking) {
        ajaxurl = '/Receive/FillPreReceiveInfoForSerialItem';
        closeOrdReceiveInfoModel();
    }
    else if (objToFillPreReceive.LotNumberTracking) {
        ajaxurl = '/Receive/FillPreReceiveInfoForLotExpItems';
        closeOrdReceiveInfoModel();
    }
    else if (!objToFillPreReceive.SerialNumberTracking && !objToFillPreReceive.LotNumberTracking && objToFillPreReceive.DateCodeTracking) {
        ajaxurl = '/Receive/FillPreReceiveInfoForLotExpItems';
        closeOrdReceiveInfoModel();
    }
    else if (objToFillPreReceive.MakePreReceiveDetail[0].IsShowNormalItemPopUp != '0') {
        var ajaxurl = '/Receive/FillPreReceiveInformation';
        closeOrdReceiveInfoModel();
    }
    else {
...
function GetToFillPreReceive(tr, aData) {
    ...
    if ($(tr).find("input[type='hidden'][id^='txtQuantityToReceive']").length > 0) {
        quantityToReceive = $(tr).find("input[type='hidden'][id^='txtQuantityToReceive']").val();
    }
    var toFillReceiveDetail = { "Quantity": RecvQty, "QuantityToReceive": parseFloat(quantityToReceive) };
    arrObj.push(toFillReceiveDetail);

    var RecvItemNumber = aData.ItemNumber;
    ...
    var RecvInTransitQty = aData.InTransitQuantity;

    var RecvItemGUID = aData.ItemGUID;
    var RecvOrderDetailGUID = aData.OrderDetailGUID;
    var RecvOrderGUID = aData.OrderGUID;
    ...
    if ($(tr).find("#txtItemCost").length > 0) {
        CurrentReceiveCost = $(tr).find("#txtItemCost").val();
    }
    if ($(tr).find("#hdItemCost").length > 0) {
        OldReceiveCost = $(tr).find("#hdItemCost").val();
    }
    if (CurrentReceiveCost != OldReceiveCost) {
        IsReceivedCostChange = true;
    }    
    /// WI-6215 Related Changes END ///

    if ($(tr).find("#txtItemCost").length > 0) {
        RecvCost = $(tr).find("#txtItemCost").val();
    }

    var RecvPackSlip = $(tr).find("input[id^='txtPackslipNo']").val();

    if (RecvPackSlip == undefined || RecvPackSlip.length <= 0) {
    ...
    var toFillPreReceive = {
        ...
        , "IsReceivedCostChange": IsReceivedCostChange, "InTransitQuantity": RecvInTransitQty
    };
