    ...
    ColReorder.prototype = {
        ...
        */
        "_fnConstruct": function () {
            ...
            //TODO: Start Updated by CP on 21-Jul-2016 for the issue WI-1832,1837
            ...
            if (this.s.dt.oLoadedState && typeof this.s.dt.oLoadedState.ColReorder != 'undefined') {
                if (this.s.dt.oLoadedState.ColReorder.length == this.s.dt.aoColumns.length) {
                    aiOrder = this.s.dt.oLoadedState.ColReorder;
                }
                else if (this.s.dt.oLoadedState.ColReorder.length > this.s.dt.aoColumns.length) {
                    aiOrder = new Array();
                    for (var i = 0; i < this.s.dt.oLoadedState.ColReorder.length; i++) {
                        if (this.s.dt.oLoadedState.ColReorder[i] < this.s.dt.aoColumns.length) {
                            aiOrder.push(this.s.dt.oLoadedState.ColReorder[i]);
                        }
                    }
                }
                else if (this.s.dt.oLoadedState.ColReorder.length < this.s.dt.aoColumns.length) {
                    ...
                    for (var i = 0; i < this.s.dt.aoColumns.length; i++) {
                        if (i >= this.s.dt.oLoadedState.ColReorder.length) {
                            aiOrder.push(i);
                        }
                        else {
                            aiOrder.push(this.s.dt.oLoadedState.ColReorder[i]);
            ...
            //TODO: END Updated by CP on 21-Jul-2016 for the issue WI-1832,1837
        ...
        */
        "_fnMouseDown": function (e, nTh, i) {
            ...
            $(document).bind('mouseup.ColReorder', function (e) {
                ...
                // END : CODE ADDED
                // commented setTimeout by amit t to fix column move issue while sort on 2-apr-20
                //setTimeout(function () { 
                ...
                if (isDragging == true) {
                    ...
                    that.s.dt.oInstance.fnAdjustColumnSizing(); // added by niraj oza to make below LINE dynemic
                }
                //}, 10);
            });
        },
            ...
            if (this.dom.resize) {
                ...
                if (this.dom.drag === null) {
                    ...
                    */
                    ...
                    /// change above formula by amit t on 31-mar-20 as below 
                    //var mouseMove = Math.pow(
                    //    Math.pow(e.pageX - this.s.mouse.startX, 2) +
                    //    Math.pow(e.pageY - this.s.mouse.startY, 2), 0.5);
                    var mouseMove = Math.pow(Math.pow(e.pageX - this.s.mouse.startX, 2), 0.5);
        ...
        */
        "_fnMouseUp": function (e, colResized) {
            ...
            if (this.dom.drag !== null) {
                ...
                // commented by Niraj for Saving proper order of column in database. By Niraj
                //this._fnConstruct();
                ///////////
            ...
            //Martin Marchetta
                ...
                //TODO: Changes By CP: Dated: 07/04/2014 for Col width Resize Save.
                ...
                var newIdx = 0;
                for (var i = 0; i < this.s.dt.aoColumns.length; i++) {
                    if ($.trim(this.s.dt.aoColumns[i].sTitle) === $.trim(this.s.mouse.resizeElem[0].textContent)) {
                        newIdx = i;
                    }
                }

                this.s.dt.aoColumns[newIdx].sWidth = $(this.s.mouse.resizeElem).innerWidth() + "px";
                //TODO: Start Updated by CP on 20-Feb-2018 for the issue WI-3587 sorting enabled on resize or reorder
                //this.s.dt.aoColumns[newIdx].bSortable = true;
                //alert(this.s.dt.aoColumns[newIdx].sClass.indexOf('nosort'));
                if (this.s.dt.aoColumns[newIdx].sClass.indexOf('nosort') < 0)
                    this.s.dt.aoColumns[newIdx].bSortable = true;
